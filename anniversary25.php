...
$_SESSION['ref'] = 0;
$comp = 'SELECT name, name_eng, date FROM competition WHERE short = "'.$_SESSION['short'].'"';
...
$name = 'SELECT name, name_chi, code FROM club WHERE username = "'.$username.'"';
...
$play = 'SELECT id, name, name_chi, birthday, gender,  active_member  FROM participants_local WHERE club = "'.$username.'" AND category = "Athlete" ORDER BY name';
...
<form name="myForm" method="POST" action="<?=$_SERVER['PHP_SELF'];?>">
	<div class = "row text-center">
		...
		?>
		   ...
		   <div class = "row text-center">
        <br>請<a href="comp_quota.php?short=<?echo $_SESSION['short']?>" target="_blank">按此</a>確認每個組別的報名人數，謝謝。
        <p>Please <a href="comp_quota.php?short=<?echo $_SESSION['short']?>" target="_blank">press here</a> to view the no. of participants per category . Thank you.</p>
  	</div>
