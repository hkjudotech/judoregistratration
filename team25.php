...
$CLUB_QUERY_TEXT = 'SELECT code,name, name_chi FROM club WHERE username = "'.$username.'"';
$club_query = mysql_query($CLUB_QUERY_TEXT) or die('Error! ' . mysql_error());
$clubname = mysql_fetch_array($club_query);
...
$result = mysql_query('SELECT id, name, name_chi, birthday, gender, active_member  FROM participants_local WHERE club = "'.$username.'" and category = "Athlete" ORDER BY name');

while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
  
  //put result row into assoc array    
  $players[$row['id']] = $row;
...
if ($DEBUG){
    foreach($players as $value){
        printf ("ID: %s  Name: %s  Member: %s   Age: %s :::::", $value["id"], $value["name"], $value["active_member"], yearAge($value["birthday"],$YEAR));
        
    }
    
    printf(">>>>>>>>>>>>>>>>>>>>");
  //  printf("************ %s", $players['10000007']["name"]);
