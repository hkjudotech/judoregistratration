...
<table id="example" class="table table-hover table-condensed table-striped table-responsive">
    ...
    <tbody>
        ...
        $stmt = $pdo->prepare('SELECT distinct competition, code, country, name, name_chi, gender, division, weight, identity, date, payment, school, custom_id FROM local  WHERE competition = ?');
        $stmt->execute([$_SESSION['short']]);
        ...
        ?>
            <tr>
                ...
                <td><?php echo ($comp['identity']) ?></td>
                <td><?php echo ($comp['payment']) ?></td>
                <td><?php echo ($comp['custom_id']) ?></td>
                <td><?php echo ($club[$comp['code']]['rep_name']) ?></td>
                <td><?php echo ($club[$comp['code']]['rep_email'])  ?></td>
                <td><?php echo ($club[$comp['code']]['rep_phone']) ?></td>
            </tr>
        <?php } ?>
    </tbody>
    <tfoot>
