...
function confirm()
{
	foreach($_SESSION['cat'] as $cat){
		$confirm = 'INSERT INTO participants_'.$_SESSION['category'].'(club, club_name, name, name_chi, gender, birthday, address, hkid, email, phone, date, category) VALUES ('.$_SESSION['add'].',"'.$cat.'")';
//		echo $confirm;
	...
	{
		 if (!file_exists("photos/".$_SESSION['username']))
		 {
			mkdir("photos/".$_SESSION['username'], 0777, true);
		}
		rename("tempupload/".$_SESSION["filename"],"photos/".$_SESSION['username']."/".trim($_SESSION['name']," -").".".$_SESSION['exten']);
	}
	$_SESSION['adding'] = 0;
	echo '<meta http-equiv=REFRESH CONTENT=1;url=add.php>';
}
