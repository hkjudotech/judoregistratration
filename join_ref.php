...
try {
    ...
    $stmt = $pdo->prepare('SELECT name, name_eng, deadline FROM competition WHERE short = ?');
    ...
    $stmt = $pdo->prepare('SELECT id, name, name_chi, category, gender, birthday 
    ...
    while ($play3 = $stmt->fetch()) {
        ...
        echo "temp[".$i."][0] = '".htmlspecialchars($play3['id'])."';";
